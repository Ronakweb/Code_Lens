...
export default function Home() {
    ...
    const handleAnalyze = async (e: React.FormEvent) => {
        e.preventDefault()
        if (!url) {
            errorHandler.handleError(new Error('Please paste a repository URL.'))
            return
        }
        ...
        try {
            const response = await errorHandler.execute(() => api.post('/analyze', { url }))
            const repoId = response.data.repo_id
            router.push(`/report/${repoId}`)
        } catch (error) {
            // Error is already handled by errorHandler.execute
        } finally {
            setLoading(false)
        }
    }
    ...
    const handleRetry = async () => {
        ...
        setLoading(true)
        try {
            await errorHandler.executeWithRetry(() => api.post('/analyze', { url }))
                                ...
                                >
                                    <ErrorFallback
                                        ...
                                        type={errorHandler.isNetworkError ? 'network' : 
                                              ...
                                              errorHandler.isTimeoutError ? 'timeout' : 'generic'}
